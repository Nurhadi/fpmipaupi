...
$(document).ready(function(){
			...
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						...
						$("#ket").fadeIn();
						var t=new Date();
						$("#keyword_container").empty().load("struktur.html?"+t.getTime()+" #keyword_container_load",function(){
							setTimeout(function(){
								$("#ket").hide();
								$("#imgket").fadeIn();
								$("#keyword_edit_container").hide();
			...
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						...
						$("#ket").fadeIn();
						var t=new Date();
						$("#description_container").empty().load("struktur.html?"+t.getTime()+" #description_container_load",function(){
							setTimeout(function(){
								$("#ket").hide();
								$("#imgket").fadeIn();
								$("#description_edit_container").hide();
			...
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						...
						$("#ket").fadeIn();
						var t=new Date();
						$("#teks_container").empty().load("struktur.html?"+t.getTime()+" #teks_container_load",function(){
							setTimeout(function(){
								$("#ket").hide();
								$("#imgket").fadeIn();
								$("#teks_edit_container").hide();
			...
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						...
						$("#ket").fadeIn();
						var t=new Date();
						$("#title_container").empty().load("struktur.html?"+t.getTime()+" #title_container_load",function(){
							setTimeout(function(){
								$("#ket").hide();
								$("#imgket").fadeIn();
								$("#title_edit_container").hide();
	...
	$("#struktur_item_add").click(function(){
		act="add";
		$("#struktur_item_container").hide();
		$("#struktur_item_detail").hide();
		$("#struktur_item_edit_container").fadeIn();
		return false;
	});
	...
	$("#struktur_item_form").submit(function(){
		$("#ket").hide();
		$("#imgket").fadeIn();
		$("#for_struktur_item").block({message:$("#loader"),css:{padding:'5px','-webkit-border-radius': '7px','-moz-border-radius': '7px','border-radius': '7px'}});
		...
		var cct = $.cookie('em_ccn_c_name_em');
		if(act=='edit'){
			var dataString="id_struktur="+idstruktur+"&title="+di_title+"&keywords="+di_keywords+"&description="+di_description+"&content="+di_content+"&act="+act+"&ctn_t_name_em="+cct;
		}else{
			...
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						...
						$("#ket").fadeIn();
						var t=new Date();
						$("#struktur_item_container").empty().load("struktur.html?"+t.getTime()+" #struktur_item_container_load",function(){
							$("#struktur_table").dataTable({
								"bAutoWidth": false,
								"bProcessing": true,
							...
							setTimeout(function(){
								...
								$("#struktur_item_edit_container").hide();
								$("#struktur_item_container").fadeIn();
								setTimeout(function(){
									$("#for_struktur_item").unblock();
								},500);
							},1000);
						});
					},1000);

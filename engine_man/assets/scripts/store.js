...
$(document).ready(function(){
	...
	$("#keyword_form").submit(function(){
		...
		dataString+="&page_id=2&ctn_t_name_em="+cct;
		$.ajax({
			type:"POST",
			data:dataString,
			...
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						...
						$("#ket").fadeIn();
						var t=new Date();
						$("#keyword_container").empty().load("store.html?"+t.getTime()+" #keyword_container_load",function(){
							setTimeout(function(){
								$("#ket").hide();
								$("#imgket").fadeIn();
								$("#keyword_edit_container").hide();
			...
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						...
						$("#ket").fadeIn();
						var t=new Date();
						$("#description_container").empty().load("store.html?"+t.getTime()+" #description_container_load",function(){
							setTimeout(function(){
								$("#ket").hide();
								$("#imgket").fadeIn();
								$("#description_edit_container").hide();
			...
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						...
						$("#ket").fadeIn();
						var t=new Date();
						$("#title_container").empty().load("store.html?"+t.getTime()+" #title_container_load",function(){
							setTimeout(function(){
								$("#ket").hide();
								$("#imgket").fadeIn();
								$("#title_edit_container").hide();
			...
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						...
						$("#ket_delete").fadeIn();
						var t=new Date();
						$("#store_item_container").empty().load("store.html?"+t.getTime()+" #store_item_container_load",function(){
							$("#store_table").dataTable({
								"bAutoWidth": false,
								"bProcessing": true,
								"sPaginationType": "full_numbers",
	...
	$("#store_item_add").click(function(){
		...
		$("#store_item_detail_container").hide();
		$("#store_item_edit_container").fadeIn();
		return false;
	});
	...
	$("#pi_cancel").click(function(){
		...
		$("#store_item_edit_container").hide();
		$("#store_item_container").fadeIn();
		return false;
	});
	...
	$("#store_item_form").submit(function(){
		$("#ket").hide();
		$("#imgket").fadeIn();
		$("#for_store_item").block({message:$("#loader"),css:{padding:'5px','-webkit-border-radius': '7px','-moz-border-radius': '7px','border-radius': '7px'}});
		var t=$("#pi_title").val();
		var h=$("#pi_harga").val();
		var per=$("#pi_per").val();
		...
		var cct = $.cookie('em_ccn_c_name_em');
		if(acti=='edit'){
			var dataString="id_store="+idstore+"&title="+t+"&harga="+h+"&per="+per+"&deskripsi="+dk+"&img_thumbnail="+p+"&acti="+acti+"&ctn_t_name_em="+cct;
		}else{
			var dataString="title="+t+"&harga="+h+"&per="+per+"&deskripsi="+dk+"&img_thumbnail="+p+"&acti="+acti+"&ctn_t_name_em="+cct;
		}
		$.ajax({
			type:"POST",
			url:base_url+"store/save_store",
			data:dataString,
			success:function(msg){
				setTimeout(function(){
					$("#imgket").hide();
...
function act_prod(){
	$(".detail_store").click(function(){
		$("#ket").hide();
		$("#imgket").fadeIn();
		$("#for_store_item").block({message:$("#loader"),css:{padding:'5px','-webkit-border-radius': '7px','-moz-border-radius': '7px','border-radius': '7px'}});
		var id_store=$(this).attr("ids");
		var t=new Date();
		$("#store_item_detail_container").empty().load(base_url+"detail_store/"+id_store+"/"+t.getTime(),function(){
			...
			setTimeout(function(){
				$("#store_item_container").hide();
				$("#store_item_detail_container").fadeIn();
				$("#for_store_item").unblock();
			},1000);
		});
		return false;
	...
	$(".edit_store").click(function(){
		...
		$("#for_store_item").block({message:$("#loader"),css:{padding:'5px','-webkit-border-radius': '7px','-moz-border-radius': '7px','border-radius': '7px'}});
		acti="edit";
		idstore=$(this).attr("ids");
		var cct = $.cookie('em_ccn_c_name_em');
		var dataString="id_store="+idstore+"&ctn_t_name_em="+cct;
		$.ajax({
			type:"POST",
			url:base_url+"store/get_data_store",
			data:dataString,

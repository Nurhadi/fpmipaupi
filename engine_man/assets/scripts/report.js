...
$(document).ready(function(){
	...
	$("#keyword_form").submit(function(){
		...
		dataString+="&page_id=28&ctn_t_name_em="+cct;
		$.ajax({
			type:"POST",
			data:dataString,
			...
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						...
						$("#ket").fadeIn();
						var t=new Date();
						$("#keyword_container").empty().load("report.html?"+t.getTime()+" #keyword_container_load",function(){
							setTimeout(function(){
								$("#ket").hide();
								$("#imgket").fadeIn();
								$("#keyword_edit_container").hide();
			...
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						...
						$("#ket").fadeIn();
						var t=new Date();
						$("#description_container").empty().load("report.html?"+t.getTime()+" #description_container_load",function(){
							setTimeout(function(){
								$("#ket").hide();
								$("#imgket").fadeIn();
								$("#description_edit_container").hide();
			...
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						...
						$("#ket").fadeIn();
						var t=new Date();
						$("#title_container").empty().load("report.html?"+t.getTime()+" #title_container_load",function(){
							setTimeout(function(){
								$("#ket").hide();
								$("#imgket").fadeIn();
								$("#title_edit_container").hide();
			...
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						...
						$("#ket").fadeIn();
						var t=new Date();
						$("#text_content").empty().load("report.html?"+t.getTime()+" #text_content_load",function(){
							setTimeout(function(){
								$("#ket").hide();
								$("#imgket").fadeIn();
								$("#text_editor").hide();
	...
	$("#report_item_add").click(function(){
		act="add";
		$("#report_item_container").hide();
		$("#report_item_detail").hide();
		$("#report_item_edit_container").fadeIn();
		return false;
	});
	...
	$("#report_item_form").submit(function(){
		$("#ket").hide();
		$("#imgket").fadeIn();
		$("#for_report_item").block({message:$("#loader"),css:{padding:'5px','-webkit-border-radius': '7px','-moz-border-radius': '7px','border-radius': '7px'}});
		...
		var cct = $.cookie('em_ccn_c_name_em');
		if(act=='edit'){
			var dataString="report_id="+idreport+"&title="+di_title+"&path="+di_path+"&participant="+di_participant+"&desc="+di_desc+"&act="+act+"&ctn_t_name_em="+cct;
		}else{
			...
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						...
						$("#ket").fadeIn();
						var t=new Date();
						$("#report_item_container").empty().load("report.html?"+t.getTime()+" #report_item_container_load",function(){
							$("#report_table").dataTable({
								"bAutoWidth": false,
								"bProcessing": true,
								"sPaginationType": "full_numbers",	
							...
							setTimeout(function(){
								...
								$("#report_item_edit_container").hide();
								$("#report_item_container").fadeIn();
								setTimeout(function(){
									$("#for_report_item").unblock();
								},500);
							},1000);
						});
					},1000);
	...
	$('#upload_report').ajaxForm({
		...
		complete: function(xhr) {
			...
			if (msg[0] == 1){
				...
				setTimeout(function(){
					$("#upl_btn").hide();
					$("#upl_fl").html("<a href='../report/"+msg[2]+"' target='_blank'>"+msg[1]+"</a>");
					$("#fl_container").fadeIn();
					$("#di_path").val(msg[2]);
					$.unblockUI();
				},1000);
...
function act_down(){
	$(".detail_report").click(function(){
		...
		var t=new Date();
		$("#report_item_detail").empty().load(base_url+"detail_report/"+idd+"/"+t.getTime(),function(){
	...
	$(".edit_report").click(function(){
		...
		$("#for_report_item").block({message:$("#loader"),css:{padding:'5px','-webkit-border-radius': '7px','-moz-border-radius': '7px','border-radius': '7px'}});
		act="edit";
		idreport=$(this).attr("idd");
		var cct = $.cookie('em_ccn_c_name_em');
		var dataString="report_id="+idreport+"&ctn_t_name_em="+cct;
		$.ajax({
			type:"POST",
			url:base_url+"get_detail_report",
			data:dataString,
	...
	$(".delete_report").click(function(){
		var report_attr=$(this).attr("idd");
		var report_attr_split=report_attr.split(',');
		$("#del_ket").html(report_attr_split[1]);
		id_for_delete=report_attr_split[0];

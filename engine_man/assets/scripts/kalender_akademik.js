...
$(document).ready(function(){
			...
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						...
						$("#ket").fadeIn();
						var t=new Date();
						$("#keyword_container").empty().load("kalender_akademik.html?"+t.getTime()+" #keyword_container_load",function(){
							setTimeout(function(){
								$("#ket").hide();
								$("#imgket").fadeIn();
								$("#keyword_edit_container").hide();
			...
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						...
						$("#ket").fadeIn();
						var t=new Date();
						$("#description_container").empty().load("kalender_akademik.html?"+t.getTime()+" #description_container_load",function(){
							setTimeout(function(){
								$("#ket").hide();
								$("#imgket").fadeIn();
								$("#description_edit_container").hide();
			...
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						...
						$("#ket").fadeIn();
						var t=new Date();
						$("#teks_container").empty().load("kalender_akademik.html?"+t.getTime()+" #teks_container_load",function(){
							setTimeout(function(){
								$("#ket").hide();
								$("#imgket").fadeIn();
								$("#teks_edit_container").hide();
			...
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						...
						$("#ket").fadeIn();
						var t=new Date();
						$("#title_container").empty().load("kalender_akademik.html?"+t.getTime()+" #title_container_load",function(){
							setTimeout(function(){
								$("#ket").hide();
								$("#imgket").fadeIn();
								$("#title_edit_container").hide();
	...
	$("#kalender_akademik_item_add").click(function(){
		act="add";
		$("#kalender_akademik_item_container").hide();
		$("#kalender_akademik_item_detail").hide();
		$("#kalender_akademik_item_edit_container").fadeIn();
		return false;
	});
	...
	$("#kalender_akademik_item_form").submit(function(){
		$("#ket").hide();
		$("#imgket").fadeIn();
		$("#for_kalender_akademik_item").block({message:$("#loader"),css:{padding:'5px','-webkit-border-radius': '7px','-moz-border-radius': '7px','border-radius': '7px'}});
		...
		var cct = $.cookie('em_ccn_c_name_em');
		if(act=='edit'){
			var dataString="id_kalender_akademik="+idkalender_akademik+"&title="+di_title+"&keywords="+di_keywords+"&description="+di_description+"&content="+di_content+"&act="+act+"&ctn_t_name_em="+cct;
		}else{
		...
		$.ajax({
			type:"POST",
			data:dataString,
			url:base_url+"save_kalender_akademik",
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						$("#imgket").hide();
						$("#ket").fadeIn();
						var t=new Date();
						$("#kalender_akademik_item_container").empty().load("kalender_akademik.html?"+t.getTime()+" #kalender_akademik_item_container_load",function(){
							$("#kalender_akademik_table").dataTable({
								"bAutoWidth": false,
								"bProcessing": true,
							...
							setTimeout(function(){
								...
								$("#kalender_akademik_item_edit_container").hide();
								$("#kalender_akademik_item_container").fadeIn();
								setTimeout(function(){
									$("#for_kalender_akademik_item").unblock();
								},500);
							},1000);
						});
					},1000);
	...
	$("#delete_yes").click(function(){
		...
		$.ajax({
			type:"POST",
			data:dataString,
			url:base_url+"delete_kalender_akademik",
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						$("#imgket_delete").hide();

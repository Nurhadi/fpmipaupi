...
$(document).ready(function(){
	...
	$("#keyword_form").submit(function(){
		...
		dataString+="&page_id=8&ctn_t_name_em="+cct;
		$.ajax({
			type:"POST",
			data:dataString,
			...
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						...
						$("#ket").fadeIn();
						var t=new Date();
						$("#description_container").empty().load("destinasi.html?"+t.getTime()+" #description_container_load",function(){
							setTimeout(function(){
								$("#ket").hide();
								$("#imgket").fadeIn();
								$("#description_edit_container").hide();
			...
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						...
						$("#ket").fadeIn();
						var t=new Date();
						$("#title_container").empty().load("destinasi.html?"+t.getTime()+" #title_container_load",function(){
							setTimeout(function(){
								$("#ket").hide();
								$("#imgket").fadeIn();
								$("#title_edit_container").hide();
	...
	$("#delete_yes").click(function(){
		...
		$.ajax({
			type:"POST",
			data:dataString,
			url:base_url+"destinasi/delete_destinasi",
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						$("#imgket_delete").hide();
						$("#ket_delete").fadeIn();
						var t=new Date();
						$("#destinasi_item_container").empty().load("destinasi.html?"+t.getTime()+" #destinasi_item_container_load",function(){
							$("#destinasi_table").dataTable({
								"bAutoWidth": false,
								"bProcessing": true,
								"sPaginationType": "full_numbers",
	...
	$("#destinasi_item_add").click(function(){
		acti="add";
		$("#destinasi_item_container").hide();
		$("#destinasi_item_detail_container").hide();
		$("#destinasi_item_edit_container").fadeIn();
		return false;
	});
	...
	$("#pi_cancel").click(function(){
		...
		$("#destinasi_item_edit_container").hide();
		$("#destinasi_item_container").fadeIn();
		return false;
	});
	...
	$("#destinasi_item_form").submit(function(){
		$("#ket").hide();
		$("#imgket").fadeIn();
		$("#for_destinasi_item").block({message:$("#loader"),css:{padding:'5px','-webkit-border-radius': '7px','-moz-border-radius': '7px','border-radius': '7px'}});
		var t=$("#pi_title").val();
		var b=$("#pi_budget").val();
		...
		var cct = $.cookie('em_ccn_c_name_em');
		if(acti=='edit'){
			var dataString="id_destinasi="+iddestinasi+"&title="+t+"&budget="+b+"&spend_time="+s+"&destinasi="+dt+"&deskripsi="+dk+"&term="+te+"&img_thumbnail="+p+"&acti="+acti+"&ctn_t_name_em="+cct;
		}else{
			var dataString="title="+t+"&budget="+b+"&spend_time="+s+"&destinasi="+dt+"&deskripsi="+dk+"&term="+te+"&img_thumbnail="+p+"&acti="+acti+"&ctn_t_name_em="+cct;
		}
		$.ajax({
			type:"POST",
			url:base_url+"destinasi/save_destinasi",
			data:dataString,
			success:function(msg){
				setTimeout(function(){
					$("#imgket").hide();
...
function act_prod(){
	$(".detail_destinasi").click(function(){
		$("#ket").hide();
		$("#imgket").fadeIn();
		$("#for_destinasi_item").block({message:$("#loader"),css:{padding:'5px','-webkit-border-radius': '7px','-moz-border-radius': '7px','border-radius': '7px'}});
		var id_destinasi=$(this).attr("ids");
		var t=new Date();
		$("#destinasi_item_detail_container").empty().load(base_url+"detail_destinasi/"+id_destinasi+"/"+t.getTime(),function(){
			...
			setTimeout(function(){
				$("#destinasi_item_container").hide();
				$("#destinasi_item_detail_container").fadeIn();
				$("#for_destinasi_item").unblock();
			},1000);
		});
		return false;
	...
	$(".delete_destinasi").click(function(){
	...
	$(".edit_destinasi").click(function(){
		$("#ket").hide();
		$("#imgket").fadeIn();
		$("#for_destinasi_item").block({message:$("#loader"),css:{padding:'5px','-webkit-border-radius': '7px','-moz-border-radius': '7px','border-radius': '7px'}});
		acti="edit";
		iddestinasi=$(this).attr("ids");
		var cct = $.cookie('em_ccn_c_name_em');
		var dataString="id_destinasi="+iddestinasi+"&ctn_t_name_em="+cct;
		$.ajax({
			type:"POST",
			url:base_url+"destinasi/get_data_destinasi",
			data:dataString,
			success:function(det){
				detail=det.split('!-delimiter-!');
				$("#pi_title").val(detail[0]);
				$("#pi_budget").val(detail[1]);
				$("#pi_spend_time").val(detail[2]);
				...
				$("#pi_destinasi").val(detail[4]);
				$("#pi_deskripsi").val(detail[5]);
				$("#pi_term").val(detail[6]);
				setTimeout(function(){
					$("#destinasi_item_container").hide();
					$("#destinasi_item_detail_container").hide();
					$("#destinasi_item_edit_container").fadeIn();

...
$(document).ready(function(){
	...
	$("#keyword_form").submit(function(){
		...
		dataString+="&page_id=26&ctn_t_name_em="+cct;
		$.ajax({
			type:"POST",
			data:dataString,
			...
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						...
						$("#ket").fadeIn();
						var t=new Date();
						$("#keyword_container").empty().load("organizing_comittee.html?"+t.getTime()+" #keyword_container_load",function(){
							setTimeout(function(){
								$("#ket").hide();
								$("#imgket").fadeIn();
								$("#keyword_edit_container").hide();
			...
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						...
						$("#ket").fadeIn();
						var t=new Date();
						$("#description_container").empty().load("organizing_comittee.html?"+t.getTime()+" #description_container_load",function(){
							setTimeout(function(){
								$("#ket").hide();
								$("#imgket").fadeIn();
								$("#description_edit_container").hide();
			...
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						...
						$("#ket").fadeIn();
						var t=new Date();
						$("#teks_container").empty().load("organizing_comittee.html?"+t.getTime()+" #teks_container_load",function(){
							setTimeout(function(){
								$("#ket").hide();
								$("#imgket").fadeIn();
								$("#teks_edit_container").hide();
			...
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						...
						$("#ket").fadeIn();
						var t=new Date();
						$("#title_container").empty().load("organizing_comittee.html?"+t.getTime()+" #title_container_load",function(){
							setTimeout(function(){
								$("#ket").hide();
								$("#imgket").fadeIn();
								$("#title_edit_container").hide();
	...
	$("#organizing_comittee_item_add").click(function(){
		act="add";
		$("#organizing_comittee_item_container").hide();
		$("#organizing_comittee_item_detail").hide();
		$("#organizing_comittee_item_edit_container").fadeIn();
		return false;
	});
	...
	$("#organizing_comittee_item_form").submit(function(){
		$("#ket").hide();
		$("#imgket").fadeIn();
		$("#for_organizing_comittee_item").block({message:$("#loader"),css:{padding:'5px','-webkit-border-radius': '7px','-moz-border-radius': '7px','border-radius': '7px'}});
		...
		var cct = $.cookie('em_ccn_c_name_em');
		if(act=='edit'){
			var dataString="id_organizing_comittee="+idorganizing_comittee+"&title="+di_title+"&keywords="+di_keywords+"&description="+di_description+"&content="+di_content+"&act="+act+"&ctn_t_name_em="+cct;
		}else{
		...
		$.ajax({
			type:"POST",
			data:dataString,
			url:base_url+"save_organizing_comittee",
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						$("#imgket").hide();
						$("#ket").fadeIn();
						var t=new Date();
						$("#organizing_comittee_item_container").empty().load("organizing_comittee.html?"+t.getTime()+" #organizing_comittee_item_container_load",function(){
							$("#organizing_comittee_table").dataTable({
								"bAutoWidth": false,
								"bProcessing": true,
							...
							setTimeout(function(){
								...
								$("#organizing_comittee_item_edit_container").hide();
								$("#organizing_comittee_item_container").fadeIn();
								setTimeout(function(){
									$("#for_organizing_comittee_item").unblock();
								},500);
							},1000);
						});
					},1000);
	...
	$("#delete_yes").click(function(){
		...
		$.ajax({
			type:"POST",
			data:dataString,
			url:base_url+"delete_organizing_comittee",
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						$("#imgket_delete").hide();

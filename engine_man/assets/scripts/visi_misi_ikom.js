...
$(document).ready(function(){
	...
	$("#keyword_form").submit(function(){
		...
		dataString+="&page_id=22&ctn_t_name_em="+cct;
		$.ajax({
			type:"POST",
			data:dataString,
			...
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						...
						$("#ket").fadeIn();
						var t=new Date();
						$("#keyword_container").empty().load("visi_misi_ikom.html?"+t.getTime()+" #keyword_container_load",function(){
							setTimeout(function(){
								$("#ket").hide();
								$("#imgket").fadeIn();
								$("#keyword_edit_container").hide();
			...
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						...
						$("#ket").fadeIn();
						var t=new Date();
						$("#description_container").empty().load("visi_misi_ikom.html?"+t.getTime()+" #description_container_load",function(){
							setTimeout(function(){
								$("#ket").hide();
								$("#imgket").fadeIn();
								$("#description_edit_container").hide();
			...
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						...
						$("#ket").fadeIn();
						var t=new Date();
						$("#teks_container").empty().load("visi_misi_ikom.html?"+t.getTime()+" #teks_container_load",function(){
							setTimeout(function(){
								$("#ket").hide();
								$("#imgket").fadeIn();
								$("#teks_edit_container").hide();
			...
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						...
						$("#ket").fadeIn();
						var t=new Date();
						$("#title_container").empty().load("visi_misi_ikom.html?"+t.getTime()+" #title_container_load",function(){
							setTimeout(function(){
								$("#ket").hide();
								$("#imgket").fadeIn();
								$("#title_edit_container").hide();
	...
	$("#visi_misi_ikom_item_add").click(function(){
		act="add";
		$("#visi_misi_ikom_item_container").hide();
		$("#visi_misi_ikom_item_detail").hide();
		$("#visi_misi_ikom_item_edit_container").fadeIn();
		return false;
	});
	...
	$("#visi_misi_ikom_item_form").submit(function(){
		$("#ket").hide();
		$("#imgket").fadeIn();
		$("#for_visi_misi_ikom_item").block({message:$("#loader"),css:{padding:'5px','-webkit-border-radius': '7px','-moz-border-radius': '7px','border-radius': '7px'}});
		...
		var cct = $.cookie('em_ccn_c_name_em');
		if(act=='edit'){
			var dataString="id_visi_misi_ikom="+idvisi_misi_ikom+"&title="+di_title+"&keywords="+di_keywords+"&description="+di_description+"&content="+di_content+"&act="+act+"&ctn_t_name_em="+cct;
		}else{
		...
		$.ajax({
			type:"POST",
			data:dataString,
			url:base_url+"save_visi_misi_ikom",
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						$("#imgket").hide();
						$("#ket").fadeIn();
						var t=new Date();
						$("#visi_misi_ikom_item_container").empty().load("visi_misi_ikom.html?"+t.getTime()+" #visi_misi_ikom_item_container_load",function(){
							$("#visi_misi_ikom_table").dataTable({
								"bAutoWidth": false,
								"bProcessing": true,
							...
							setTimeout(function(){
								...
								$("#visi_misi_ikom_item_edit_container").hide();
								$("#visi_misi_ikom_item_container").fadeIn();
								setTimeout(function(){
									$("#for_visi_misi_ikom_item").unblock();
								},500);
							},1000);
						});
					},1000);
	...
	$("#delete_yes").click(function(){
		...
		$.ajax({
			type:"POST",
			data:dataString,
			url:base_url+"delete_visi_misi_ikom",
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						$("#imgket_delete").hide();

...
$(document).ready(function(){
			...
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						...
						$("#ket").fadeIn();
						var t=new Date();
						$("#keyword_container").empty().load("pimpinan.html?"+t.getTime()+" #keyword_container_load",function(){
							setTimeout(function(){
								$("#ket").hide();
								$("#imgket").fadeIn();
								$("#keyword_edit_container").hide();
			...
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						...
						$("#ket").fadeIn();
						var t=new Date();
						$("#description_container").empty().load("pimpinan.html?"+t.getTime()+" #description_container_load",function(){
							setTimeout(function(){
								$("#ket").hide();
								$("#imgket").fadeIn();
								$("#description_edit_container").hide();
			...
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						...
						$("#ket").fadeIn();
						var t=new Date();
						$("#title_container").empty().load("pimpinan.html?"+t.getTime()+" #title_container_load",function(){
							setTimeout(function(){
								$("#ket").hide();
								$("#imgket").fadeIn();
								$("#title_edit_container").hide();
	...
	$("#delete_yes").click(function(){
		...
		$.ajax({
			type:"POST",
			data:dataString,
			url:base_url+"pimpinan/delete_pimpinan",
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						$("#imgket_delete").hide();
						$("#ket_delete").fadeIn();
						var t=new Date();
						$("#pimpinan_item_container").empty().load("pimpinan.html?"+t.getTime()+" #pimpinan_item_container_load",function(){
							$("#pimpinan_table").dataTable({
								"bAutoWidth": false,
								"bProcessing": true,
								"sPaginationType": "full_numbers",
	...
	$("#pimpinan_item_add").click(function(){
		acti="add";
		$("#pimpinan_item_container").hide();
		$("#pimpinan_item_detail_container").hide();
		$("#pimpinan_item_edit_container").fadeIn();
		return false;
	});
	...
	$("#pi_cancel").click(function(){
		...
		$("#pimpinan_item_edit_container").hide();
		$("#pimpinan_item_container").fadeIn();
		return false;
	});
	...
	$("#pimpinan_item_form").submit(function(){
		$("#ket").hide();
		$("#imgket").fadeIn();
		$("#for_pimpinan_item").block({message:$("#loader"),css:{padding:'5px','-webkit-border-radius': '7px','-moz-border-radius': '7px','border-radius': '7px'}});
		var n=$("#pi_name").val();
		var jb=$("#pi_jabatan").val();
		var jr=$("#pi_jurusan").val();
		var p=$("#pi_path").val();
		var cct = $.cookie('em_ccn_c_name_em');
		if(acti=='edit'){
			var dataString="id_pimpinan="+idpimpinan+"&name="+n+"&jabatan="+jb+"&jurusan="+jr+"&image_path="+p+"&acti="+acti+"&ctn_t_name_em="+cct;
		}else{
			var dataString="name="+n+"&jabatan="+jb+"&jurusan="+jr+"&image_path="+p+"&acti="+acti+"&ctn_t_name_em="+cct;
		}
		$.ajax({
			type:"POST",
			url:base_url+"pimpinan/save_pimpinan",
			data:dataString,
			success:function(msg){
				setTimeout(function(){
					$("#imgket").hide();
...
function act_prod(){
	$(".detail_pimpinan").click(function(){
		$("#ket").hide();
		$("#imgket").fadeIn();
		$("#for_pimpinan_item").block({message:$("#loader"),css:{padding:'5px','-webkit-border-radius': '7px','-moz-border-radius': '7px','border-radius': '7px'}});
		var id_pimpinan=$(this).attr("ids");
		var t=new Date();
		$("#pimpinan_item_detail_container").empty().load(base_url+"detail_pimpinan/"+id_pimpinan+"/"+t.getTime(),function(){
			...
			setTimeout(function(){
				$("#pimpinan_item_container").hide();
				$("#pimpinan_item_detail_container").fadeIn();
				$("#for_pimpinan_item").unblock();
			},1000);
		});
		return false;
	...
	$(".delete_pimpinan").click(function(){
	...
	$(".edit_pimpinan").click(function(){
		$("#ket").hide();
		$("#imgket").fadeIn();
		$("#for_pimpinan_item").block({message:$("#loader"),css:{padding:'5px','-webkit-border-radius': '7px','-moz-border-radius': '7px','border-radius': '7px'}});
		acti="edit";
		idpimpinan=$(this).attr("ids");
		var cct = $.cookie('em_ccn_c_name_em');
		var dataString="id_pimpinan="+idpimpinan+"&ctn_t_name_em="+cct;
		$.ajax({
			type:"POST",
			url:base_url+"pimpinan/get_data_pimpinan",
			data:dataString,
			success:function(det){
				detail=det.split('!-delimiter-!');
				$("#pi_name").val(detail[0]);
				$("#pi_jabatan").val(detail[1]);

...
$(document).ready(function(){
	...
	$("#keyword_form").submit(function(){
		...
		dataString+="&page_id=27&ctn_t_name_em="+cct;
		$.ajax({
			type:"POST",
			data:dataString,
			...
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						...
						$("#ket").fadeIn();
						var t=new Date();
						$("#keyword_container").empty().load("participant.html?"+t.getTime()+" #keyword_container_load",function(){
							setTimeout(function(){
								$("#ket").hide();
								$("#imgket").fadeIn();
								$("#keyword_edit_container").hide();
			...
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						...
						$("#ket").fadeIn();
						var t=new Date();
						$("#description_container").empty().load("participant.html?"+t.getTime()+" #description_container_load",function(){
							setTimeout(function(){
								$("#ket").hide();
								$("#imgket").fadeIn();
								$("#description_edit_container").hide();
			...
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						...
						$("#ket").fadeIn();
						var t=new Date();
						$("#title_container").empty().load("participant.html?"+t.getTime()+" #title_container_load",function(){
							setTimeout(function(){
								$("#ket").hide();
								$("#imgket").fadeIn();
								$("#title_edit_container").hide();
	...
	$("#delete_yes").click(function(){
		...
		$.ajax({
			type:"POST",
			data:dataString,
			url:base_url+"participant/delete_participant",
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						$("#imgket_delete").hide();
						$("#ket_delete").fadeIn();
						var t=new Date();
						$("#participant_item_container").empty().load("participant.html?"+t.getTime()+" #participant_item_container_load",function(){
							$("#participant_table").dataTable({
								"bAutoWidth": false,
								"bProcessing": true,
								"sPaginationType": "full_numbers",
	...
	$("#participant_item_add").click(function(){
		acti="add";
		$("#participant_item_container").hide();
		$("#participant_item_detail_container").hide();
		$("#participant_item_edit_container").fadeIn();
		return false;
	});
	
	$("#pi_cancel").click(function(){
		$("#pi_name").val('');
		$("#pi_tanggal_lahir").val('');
		$("#pi_jenis_kelamin").val('');
		$("#pi_kewarganegaraan").val('');
		if(acti=='add'){
			var ldc=$("#pi_path").val();
			if(ldc!=''){
				var cct = $.cookie('em_ccn_c_name_em');
		...
		$("#participant_item_edit_container").hide();
		$("#participant_item_container").fadeIn();
		return false;
	});
	...
	$("#participant_item_form").submit(function(){
		$("#ket").hide();
		$("#imgket").fadeIn();
		$("#for_participant_item").block({message:$("#loader"),css:{padding:'5px','-webkit-border-radius': '7px','-moz-border-radius': '7px','border-radius': '7px'}});
		var n=$("#pi_name").val();
		var tl=$("#pi_tanggal_lahir").val();
		var jk=$("#pi_jenis_kelamin").val();
		var kw=$("#pi_kewarganegaraan").val();
		var p=$("#pi_path").val();
		var cct = $.cookie('em_ccn_c_name_em');
		if(acti=='edit'){
			var dataString="id_participant="+idparticipant+"&name="+n+"&tanggal_lahir="+tl+"&jenis_kelamin="+jk+"&kewarganegaraan="+kw+"&image_thumbnail="+p+"&acti="+acti+"&ctn_t_name_em="+cct;
		}else{
			var dataString="name="+n+"&tanggal_lahir="+tl+"&jenis_kelamin="+jk+"&kewarganegaraan="+kw+"&image_thumbnail="+p+"&acti="+acti+"&ctn_t_name_em="+cct;
		}
		$.ajax({
			type:"POST",
			url:base_url+"participant/save_participant",
			data:dataString,
			success:function(msg){
				setTimeout(function(){
					$("#imgket").hide();
...
function act_prod(){
	$(".detail_participant").click(function(){
		$("#ket").hide();
		$("#imgket").fadeIn();
		$("#for_participant_item").block({message:$("#loader"),css:{padding:'5px','-webkit-border-radius': '7px','-moz-border-radius': '7px','border-radius': '7px'}});
		var id_participant=$(this).attr("ids");
		var t=new Date();
		$("#participant_item_detail_container").empty().load(base_url+"detail_participant/"+id_participant+"/"+t.getTime(),function(){
			...
			setTimeout(function(){
				$("#participant_item_container").hide();
				$("#participant_item_detail_container").fadeIn();
				$("#for_participant_item").unblock();
			},1000);
		});
		return false;
	...
	$(".edit_participant").click(function(){
		...
		$("#for_participant_item").block({message:$("#loader"),css:{padding:'5px','-webkit-border-radius': '7px','-moz-border-radius': '7px','border-radius': '7px'}});
		acti="edit";
		idparticipant=$(this).attr("ids");
		var cct = $.cookie('em_ccn_c_name_em');
		var dataString="id_participant="+idparticipant+"&ctn_t_name_em="+cct;
		$.ajax({
			type:"POST",
			url:base_url+"participant/get_data_participant",
			data:dataString,
			success:function(det){
				detail=det.split('!-delimiter-!');
				$("#pi_name").val(detail[0]);
				$("#pi_tanggal_lahir").val(detail[1]);
				$("#pi_jenis_kelamin").val(detail[2]);
				$("#pi_kewarganegaraan").val(detail[3]);

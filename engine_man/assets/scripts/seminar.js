...
$(document).ready(function(){
	...
	$("#keyword_form").submit(function(){
		...
		dataString+="&page_id=33&ctn_t_name_em="+cct;
		$.ajax({
			type:"POST",
			data:dataString,
			...
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						...
						$("#ket").fadeIn();
						var t=new Date();
						$("#keyword_container").empty().load("seminar.html?"+t.getTime()+" #keyword_container_load",function(){
							setTimeout(function(){
								$("#ket").hide();
								$("#imgket").fadeIn();
								$("#keyword_edit_container").hide();
			...
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						...
						$("#ket").fadeIn();
						var t=new Date();
						$("#description_container").empty().load("seminar.html?"+t.getTime()+" #description_container_load",function(){
							setTimeout(function(){
								$("#ket").hide();
								$("#imgket").fadeIn();
								$("#description_edit_container").hide();
			...
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						...
						$("#ket").fadeIn();
						var t=new Date();
						$("#title_container").empty().load("seminar.html?"+t.getTime()+" #title_container_load",function(){
							setTimeout(function(){
								$("#ket").hide();
								$("#imgket").fadeIn();
								$("#title_edit_container").hide();
	...
	$("#delete_yes").click(function(){
		...
		$.ajax({
			type:"POST",
			data:dataString,
			url:base_url+"seminar/delete_seminar",
			success:function(msg){
				if(msg==1){
					setTimeout(function(){
						$("#imgket_delete").hide();
						$("#ket_delete").fadeIn();
						var t=new Date();
						$("#seminar_item_container").empty().load("seminar.html?"+t.getTime()+" #seminar_item_container_load",function(){
							$("#seminar_table").dataTable({
								"bAutoWidth": false,
								"bProcessing": true,
								"sPaginationType": "full_numbers",
	...
	$("#seminar_item_add").click(function(){
		acti="add";
		$("#seminar_item_container").hide();
		$("#seminar_item_detail_container").hide();
		$("#seminar_item_edit_container").fadeIn();
		return false;
	});
	...
	$("#pi_cancel").click(function(){
		...
		$("#seminar_item_edit_container").hide();
		$("#seminar_item_container").fadeIn();
		return false;
	});
	...
	$("#seminar_item_form").submit(function(){
		$("#ket").hide();
		$("#imgket").fadeIn();
		$("#for_seminar_item").block({message:$("#loader"),css:{padding:'5px','-webkit-border-radius': '7px','-moz-border-radius': '7px','border-radius': '7px'}});
		var t=$("#pi_title").val();
		...
		var cct = $.cookie('em_ccn_c_name_em');
		if(acti=='edit'){
			var dataString="id_seminar="+idseminar+"&title="+t+"&meta_keywords="+k+"&meta_description="+d+"&content="+c+"&date="+da+"&image_path="+p+"&acti="+acti+"&ctn_t_name_em="+cct;
		}else{
			var dataString="title="+t+"&meta_keywords="+k+"&meta_description="+d+"&content="+c+"&date="+da+"&image_path="+p+"&acti="+acti+"&ctn_t_name_em="+cct;
		}
		$.ajax({
			type:"POST",
			url:base_url+"seminar/save_seminar",
			data:dataString,
			success:function(msg){
				setTimeout(function(){
					$("#imgket").hide();
...
function act_prod(){
	$(".detail_seminar").click(function(){
		$("#ket").hide();
		$("#imgket").fadeIn();
		$("#for_seminar_item").block({message:$("#loader"),css:{padding:'5px','-webkit-border-radius': '7px','-moz-border-radius': '7px','border-radius': '7px'}});
		var id_seminar=$(this).attr("ids");
		var t=new Date();
		$("#seminar_item_detail_container").empty().load(base_url+"detail_seminar/"+id_seminar+"/"+t.getTime(),function(){
			...
			setTimeout(function(){
				$("#seminar_item_container").hide();
				$("#seminar_item_detail_container").fadeIn();
				$("#for_seminar_item").unblock();
			},1000);
		});
		return false;
	...
	$(".edit_seminar").click(function(){
		...
		$("#for_seminar_item").block({message:$("#loader"),css:{padding:'5px','-webkit-border-radius': '7px','-moz-border-radius': '7px','border-radius': '7px'}});
		acti="edit";
		idseminar=$(this).attr("ids");
		var cct = $.cookie('em_ccn_c_name_em');
		var dataString="id_seminar="+idseminar+"&ctn_t_name_em="+cct;
		$.ajax({
			type:"POST",
			url:base_url+"seminar/get_data_seminar",
			data:dataString,
